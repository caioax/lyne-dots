...
check_source_exists() {
    local hyprland_conf="$HOME/.config/hypr/hyprland.lua"

    if [[ ! -f "$hyprland_conf" ]]; then
        return 1
    fi

    # Search for a require("workspaces") call, ignoring comments
    grep -v '^[[:space:]]*--' "$hyprland_conf" 2>/dev/null |
        grep -qE 'require\([[:space:]]*"workspaces"[[:space:]]*\)'
}
...
HEADER
    ...
    if check_source_exists; then
        ...
        log_info ""
        log_info "📝 Add this line to your hyprland.lua:"
        log_info "    require(\"workspaces\")"
        log_info ""
